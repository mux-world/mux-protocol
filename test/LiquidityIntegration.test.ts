...
describe("LiquidityIntegration", () => {
  ...
  it("curve - 2", async () => {
    ...
    const curveMod = await createContract("Curve2PoolFarmModule", [
      curve2pool.address, usdc.address, usdt.address, lpStake.address
    ])
    ...
    var tx1 = await liquidityManager.callGenericModule(
      toBytes32("transferFromPool"),
      U.defaultAbiCoder.encode(["uint8[]", "uint256[]"], [[0, 2], [toUnit("100", 6), toUnit("100", 6)]])
    )
    var tx2 = await liquidityManager.callDexModule(
      1,
      toBytes32("addLiquidity"),
      U.defaultAbiCoder.encode(["uint256[]", "uint256", "bytes"], [[toUnit("100", 6), toUnit("100", 6)], 99999999999, "0x"]),
    )
    console.log("    addDexLiquidity(curve): ", (await tx1.wait()).gasUsed.add((await tx2.wait()).gasUsed))
    ...
    var tx = await liquidityManager.callDexModule(
      1,
      toBytes32("removeLiquidity"),
      U.defaultAbiCoder.encode(["uint256", "uint256[]", "uint256"], [toWei("199.92"), [0, 0], 99999999999])
    )
    ...
    await liquidityManager.callGenericModule(
      toBytes32("transferToPool"),
      U.defaultAbiCoder.encode(["uint8[]", "uint256[]"], [[0, 2], [toUnit("99.969997", 6), toUnit("99.969997", 6)]])
    )
