...
describe("IntegrationSpread", () => {
  ...
  beforeEach(async () => {
    ...
    // id, symbol, imr, mmr, fee, fee, minBps, minTime, spotWeight
    await pool.setAssetParams(1, toBytes32("BTC"), rate("0.1"), rate("0.05"), rate("0.001"), rate("0.001"), rate("0.01"), 10, 2)
    // id, tradable, openable, shortable, useStable, enabled, strict, liq, halfSpread, maxLong, maxShort
    await pool.setAssetFlags(1, true, true, true, false, true, false, true, rate("0.001"), toWei("10000000"), toWei("10000000"))
